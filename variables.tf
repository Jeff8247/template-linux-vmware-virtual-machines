...
variable "template_name" {
  description = "Name of the Linux VM template to clone"
  type        = string
  default     = "template-ubuntu-22.04-lts"
}
...
variable "guest_id" {
  description = "vSphere guest OS identifier (e.g. ubuntu64Guest). Inherited from template when null."
  type        = string
  default     = null
}

variable "domain" {
  description = "DNS domain applied during customization (e.g. corp.example.com)"
  type        = string
  default     = null
}
...
variable "wait_for_guest_net_timeout" {
  description = "Minutes to wait for guest networking to come up (0 to disable)"
  type        = number
  default     = 5
}
...
variable "customize_timeout" {
  description = "Minutes to wait for customization to complete"
  type        = number
  default     = 30
}

variable "extra_config" {
  description = "Additional VMX key/value pairs"
  type        = map(string)
  default     = {}
}
